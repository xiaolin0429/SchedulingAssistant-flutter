      ...
      builder: (context, state) {
        ...
        if (state is HomeLoaded) {
          ...
          if (state.isSelectingShiftType && state.availableShiftTypes != null) {
            // 使用WidgetsBinding.instance.addPostFrameCallback来确保在构建完成后显示对话框
            WidgetsBinding.instance.addPostFrameCallback((_) {
              if (!state.isSelectingShiftType) return; // 防止重复显示
              if (!context.mounted) return; // 检查context是否还有效
              showDialog(
                context: context,
                barrierDismissible: true, // 允许点击外部关闭
                builder: (dialogContext) => ShiftTypeSelectionDialog( // 使用新的BuildContext
