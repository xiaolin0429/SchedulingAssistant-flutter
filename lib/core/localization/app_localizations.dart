...
class AppLocalizations {
  ...
  static AppLocalizations of(BuildContext context) {
    ...
    if (localizations == null) {
      debugPrint('警告: AppLocalizations未初始化，请确保在MaterialApp中添加了AppLocalizations.delegate');
      debugPrint('当前context: ${context.widget.runtimeType}, 当前locale: ${Localizations.localeOf(context)}');
  ...
  Future<bool> load() async {
    try {
      ...
      debugPrint('本地化文件加载成功，长度: ${jsonString.length}');
  ...
  String translate(String key) {
    ...
    if (value == null) {
      debugPrint('警告: 未找到本地化字符串，key: $key, 可用keys: ${_localizedStrings.keys.take(5).join(", ")}...');
      return key;
    }
    return value;
  }
...
class _AppLocalizationsDelegate extends LocalizationsDelegate<AppLocalizations> {
  // 使用工厂构造函数，确保单例
  static final _AppLocalizationsDelegate _instance = _AppLocalizationsDelegate._internal();
