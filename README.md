...
- **构建iOS应用**: 生成未签名的IPA文件
- **创建发布包**: 当推送标签时，自动创建GitHub Release并上传构建产物
...
2. 创建并推送新版本标签：
   ```bash
   git tag v1.0.0  # 使用实际版本号
   git push origin v1.0.0
   ```
3. GitHub Actions将自动构建应用并创建带有APK和IPA文件的Release草稿

### 注意事项

- 确保标签版本与`pubspec.yaml`中的版本号匹配
- Release初始创建为草稿模式，可以在发布前添加更多信息
- iOS构建产物不包含签名，需要手动签名后才能分发
